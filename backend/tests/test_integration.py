...
MOCK_GEMINI_RESPONSE = {
    ...
    "specs": {
        "net_quantity": "60 kapslí",
        "expiry_date_indicated": True
    },
    "marketing": {
        "claims": ["Energy Boost"],
        "description": "Nakopávač pro trénink."
    },
...
def test_analyze_endpoint_flow(mock_model_cls, mock_get_file, mock_upload_file):
    ...
    mock_response.text = str(MOCK_GEMINI_RESPONSE).replace("'", '"').replace("True", "true")
...
def test_sanitizer_logic_direct():
    ...
    raw_schema = {
        ...
        "properties": {
            "composition": {
                "anyOf": [{"$ref": "#/$defs/Ingredient"}, {"type": "null"}]
            }
        },
        "title": "MainModel",
        "type": "object"
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("Integration tests require pytest/httpx. Run: 'pytest backend/tests/test_integration.py'")
