...
async def list_scans(
    q: Optional[str] = Query(
        None, description="Fulltext search query (název produktu)"
    ),
    source_url: Optional[str] = Query(
        None, description="Filtrování podle přesné URL zdroje"
    ),
...
async def update_scan(
    ...
    try:
        ...
        updated_record = await run_in_threadpool(
            db_service.update_scan_data, scan_id=scan_id, update_data=clean_data
        )

        if not updated_record:
            raise HTTPException(
                status_code=404, detail="Scan not found or update failed"
            )
    ...
    try:
        ...
        try:
            ...
            analysis_result: ProductAnalysisResult = await gemini_service.analyze_image(
                temp_file.name, file.content_type
            )
            ...
            db_record = await run_in_threadpool(
                db_service.insert_scan_data, scan_data=db_payload
            )
            ...
            # -----------------------------------------------------
            logger.error(
                f"Transaction Failed ({transaction_error}). executing Rollback..."
            )
            await run_in_threadpool(db_service.delete_file, file_name=unique_filename)
