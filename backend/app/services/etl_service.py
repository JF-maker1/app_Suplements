...
class EtlService:
    ...
    def __init__(self):
        ...
        logger.info(f"ETL Service initialized with {len(self.api_keys)} API keys for rotation.")
    ...
    async def process_scan(self, scan_id: str, raw_text: str) -> bool:
        ...
        if not derived_data:
            logger.error(f"ETL: All attempts failed for {scan_id}. Last error: {last_error}")
            return False
