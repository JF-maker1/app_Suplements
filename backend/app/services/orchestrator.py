...
class OrchestratorResult(BaseModel):
    ...
    """
    type: str = Field(..., description="Typ obsahu: 'text', 'product_cards', 'data_table'")
    content: Optional[str] = Field(None, description="Textová odpověď pro uživatele")
    payload: Optional[Any] = Field(None, description="Strukturovaná data (List[Dict] nebo Dict)")
    metadata: Optional[Dict[str, Any]] = Field({}, description="Debug info: intent, latency, tool_used")
...
class OrchestratorService:
    ...
    async def process_user_query(self, query: str, history: List[Dict[str, str]] = []) -> OrchestratorResult:
        """
        Main ReAct Loop:
        1. Classify Intent
        ...
        """
        if not query:
            return OrchestratorResult(type="text", content="Omlouvám se, ale neslyšel jsem otázku.")
    ...
    async def _handle_vector_search(self, query: str, intent: str) -> OrchestratorResult:
        """Executes Semantic Search -> Returns Product Cards"""
        try:
            # Voláme robustní VectorService (již implementováno)
            products = await self.vector_service.search_vectors(query, limit=5, threshold=0.4)
            
            if not products:
                logger.info("Vector Search returned empty. Falling back to General Chat.")
            ...
            return await self._handle_general_chat(query, [], intent, system_prefix="Došlo k chybě při vyhledávání.")
    ...
    async def _handle_general_chat(
        ...
        if not self.api_keys:
            return OrchestratorResult(type="text", content="Omlouvám se, nemám spojení s AI (API Keys missing).")
        ...
        for attempt in range(max_retries):
            ...
            current_model = self.chat_model_rotation[attempt % len(self.chat_model_rotation)]

            try:
                def _chat_generate():
                    genai.configure(api_key=current_key)
                    model = genai.GenerativeModel(current_model, system_instruction=system_prompt)
        ...
        logger.error(f"Chat Handler Critical Error: All attempts failed. Last: {last_error}")
        return OrchestratorResult(
            type="text", 
