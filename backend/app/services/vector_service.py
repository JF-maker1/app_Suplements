...
class VectorService:
    ...
    def __init__(self):
        ...
        self.model_rotation = ["models/gemini-embedding-001"]
        ...
        for attempt in range(max_retries):
            ...
            try:
                ...
                if embedding and len(embedding) == 3072:
                    ...
                    logger.warning(
                        f"⚠️ Dimension Mismatch: Got {len(embedding)}, expected 3072."
                    )
                else:
                    logger.warning(
                        f"⚠️ Empty embedding returned. Model: {current_model}"
                    )
    ...
    async def search_vectors(
        ...
        try:
            ...
            logger.info(
                f"Vector Search: Found {len(results)} matches for query '{query_text}'"
            )
            return results
